...
| Option | Required? | Description |
|--------|-----------|-------------|
| `--aws-profile, -p` | No | The AWS profile to use for this command. Can also use `AWS_PROFILE`. Defaults to `default`. Can also use `AWS_PROFILE`. Defaults to `default`. Will also check `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` environment variables if not using AWS CLI. |
| `--aws-region, -r` | No | The AWS region for the integration. Can use `AWS_DEFAULT_REGION` environment variable. Defaults to AWS profile default. |
